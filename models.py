...
class Movie(db.Model):
    ...
    links = db.relationship("Links", backref="movie", lazy=True, uselist=False)
    tags = db.relationship("Tags", backref="movie", lazy=True, uselist=True)
...
class Links(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    movie_id = db.Column(db.Integer, db.ForeignKey("movies.id"), nullable=False)
    imdbId = db.Column(db.String(255), nullable=True, server_default="")
    tmdbId = db.Column(db.String(255), nullable=True, server_default="")
    imdbId_link = db.Column(db.String(255), nullable=True, server_default="")
